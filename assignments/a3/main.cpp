...
public:
	...
	void Create_Angry_Bird_Palace()
	{
		...
		auto tower = Add_Obj_Mesh_Object_From_File("tower.obj", OpenGLColor(.0f, .5f, .5f, 1.f));
		{
			...
			Matrix4f t;
			t << 2 * cos(angle), 0., 2 * -sin(angle), -6.,
				0.,     		 6., 0.,      	      3.,
				2 * sin(angle),  0., 2 * cos(angle),  0.,
				0.,     		 0., 0.,      	      1.;
		...
		for (int i = 0; i < tree2_num; i++) {
			float angle = DegreesToRadians(i * 360. / 36.);
		...
		for (int i = 0; i < bird_num; i++) {
			...
			{
				Matrix4f t;
				t << cos(angle), sin(angle), 0., -5. + 5 * time[i],
					-sin(angle), cos(angle), 0., 9.8 * time[i] - 0.5 * 9.8 * time[i] * time [i],
					0.,  		 0., 		 1., 0.,
					0., 		 0., 		 0., 1.;
	...
	void Create_Angry_Bird_Garden()
	{
		...
		auto chairA = Add_Obj_Mesh_Object_From_File("garden/chair1.obj", OpenGLColor(1.f, .7f, .2f, 1.f));
		{
			float angle = DegreesToRadians(45);
			Matrix4f t;
			t << cos(angle), 0., sin(angle), -3.,
				0., 1., 0., 0.5,
				-sin(angle), 0., cos(angle), -3.,
				0., 0., 0., 1.;

			chairA->Set_Model_Matrix(t);
		}

		auto chairB = Add_Obj_Mesh_Object_From_File("garden/chair1.obj", OpenGLColor(1.f, .7f, .2f, 1.f));
		{
			float angle = DegreesToRadians(45);
			Matrix4f t;
			t << 1., 0., 0., -2.,
			...
			chairB->Set_Model_Matrix(t);
		}

		int plant_num = 6;
		for (int i = 0; i < plant_num; i++) {
			float angle = DegreesToRadians(i * 360. / 6.);
			auto plant = Add_Obj_Mesh_Object_From_File("garden/grass6.obj", OpenGLColor(0.2, 0.5f, 0.1f, 1.f));
			{
				Matrix4f t;
				t << 1., 0., 0., 2. + 1.5 * cos(angle),
					0., 1., 0., 0.2,
					0., 0., 1., -1. + 1.5 * sin(angle),
					0., 0., 0., 1.;
				plant->Set_Model_Matrix(t);
			}
		}

		auto fountain = Add_Obj_Mesh_Object_From_File("garden/fountain1.obj", OpenGLColor(.6f, .8f, .95f, 1.f));
		{
			Matrix4f t;
			t << 1.5, 0., 0., 2.,
				0., 1.5, 0., 0.75,
		...
		auto tree1 = Add_Obj_Mesh_Object_From_File("garden/garden_tree1.obj", OpenGLColor(0.1f, .6f, 0.1f, 1.f));
		{
			Matrix4f t;
			t << 2., 0., 0., -4.,
				0., 2., 0., 1.,
				0., 0., 2., -4.,
				0., 0., 0., 1.;
			tree1->Set_Model_Matrix(t);
		}

		auto tree2_1 = Add_Obj_Mesh_Object_From_File("garden/garden_tree2.obj", OpenGLColor(0.2f, 0.4f, 0.1f, 1.f));
		{
			Matrix4f t;
			t << 2., 0., 0., 2.,
				0., 2., 0., 1.,
				0., 0., 2., -4.,
				0., 0., 0., 1.;
			tree2_1->Set_Model_Matrix(t);
		}

		auto tree2_2 = Add_Obj_Mesh_Object_From_File("garden/garden_tree2.obj", OpenGLColor(0.2f, 0.4f, 0.1f, 1.f));
		{
			Matrix4f t;
			t << 2., 0., 0., 4.,
				0., 2., 0., 1.,
				0., 0., 2., -4.,
				0., 0., 0., 1.;
			tree2_2->Set_Model_Matrix(t);
		}

		auto tree2_3 = Add_Obj_Mesh_Object_From_File("garden/garden_tree2.obj", OpenGLColor(0.2f, 0.4f, 0.1f, 1.f));
		{
			Matrix4f t;
			t << 2., 0., 0., 3.,
				0., 2., 0., 1.,
				0., 0., 2., -4.,
				0., 0., 0., 1.;
			tree2_3->Set_Model_Matrix(t);
		}

		auto table = Add_Obj_Mesh_Object_From_File("garden/table1.obj", OpenGLColor(.8f, .5f, .2f, 1.f));
		{
			Matrix4f t;
			t << 1.5, 0., 0., -1.5,
				0., 1.5, 0., .2,
				0., 0., 1.5, -2.5,
				0., 0., 0., 1.;
			table->Set_Model_Matrix(t);
		}


		for (int x = -5; x <= 5; x++) {
			for (int z = -5; z <= 2; z++) {
				auto grass = Add_Obj_Mesh_Object_From_File("garden/ground.obj", OpenGLColor(.6f, .95f, .1f, 1.f));
				{
					Matrix4f t;
					t << 1., 0., 0., x,
						0., .1, 0., 0,
