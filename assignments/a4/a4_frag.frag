...
vec4 shading_lambertian(Light light, vec3 p, vec3 s, vec3 n) 
{
    ...
    vec3 color = kd * Id * max(0, dot(l, n));
...
{
    /* your implementation starts */
    vec4 ambient = shading_ambient(light);

    vec4 lambertian = shading_lambertian(light, p, s, n);
    ...
    return (ambient + lambertian + phong);
    /* your implementation ends */
}
...
light_model_mtx = transpose(light_model_mtx);
...
void main() 
{
    ...
    // vec3 s2 = light2.position;          //// light 2 position
    ...
    const Light light3 = Light(/*position*/ vec3(0, 2, 3), 
                                /*Ia*/ vec3(0.3, 0.3, 0.35), 
                                /*Id*/ vec3(0.65, 0.6, 0.6), 
                                /*Is*/ vec3(0.5, 0.5, 0.5));

    //// variables that you may find useful for your implementation
    vec3 s3 = light3.position;          //// light position

    const Light light4 = Light(/*position*/ vec3(0, 10, -4), 
                                /*Ia*/ vec3(0.1, 0.1, 0.1), 
                                /*Id*/ vec3(0.3, 0.2, 0.2), 
                                /*Is*/ vec3(0.1, 0.1, 0.1));

    //// variables that you may find useful for your implementation
    vec3 s4 = light4.position;          //// light position

    frag_color = shading_phong(light3, e, p, s3, n) + shading_phong(light4, e, p, s4, n);
}