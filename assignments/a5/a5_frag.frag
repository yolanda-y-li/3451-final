...
vec4 shading_texture_with_checkerboard() 
{
    ...
    float cell_size = 10.; // 1x1 becomes 10x10
    
    float ix = floor(cell_size * uv.x);
    float iy = floor(cell_size * uv.y);
...
vec4 shading_texture_with_phong(Light light, vec3 e, vec3 p, vec3 s, vec3 n) 
{
    ...
    vec3 Id = light.Id;
    vec4 lambertian = vec4(kd * Id * max(0, dot(l, n)) * tex_color, 1.);
...
vec3 read_normal_texture() 
{
    ...
    normal = texture(tex_normal, uv).xyz * 2. - 1.;
