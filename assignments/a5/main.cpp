...
public:
    ...
    void Create_Old_Object_Scene()
    {
        ...
        {
            //// initialize mesh
            auto amphora = Add_Obj_Mesh_Object("./textured_mesh/amphora/amphora.obj");
            amphora->name = "amphora"; //// Must set name for the object

            //// initialize transform
            Matrix4f t;
            t << 1.5 * cos(M_PI / 2), 0, 1.5 * sin(M_PI / 2), 0,
                0, 1.5, 0, 0.,
                -1.5 * sin(M_PI / 2), 0, 1.5 * cos(M_PI / 2), 0,
                0, 0, 0, 1;
            amphora->Set_Model_Matrix(t);

            //// initialize material
            amphora->Set_Ka(Vector3f(0.1, 0.1, 0.1));
            amphora->Set_Kd(Vector3f(0.7, 0.7, 0.7));
            amphora->Set_Ks(Vector3f(.5, .5, .5));
            amphora->Set_Shininess(15.);

            //// initialize texture
            Add_Textture_For_Mesh_Object(amphora, "./textured_mesh/amphora/amphora.jpg", TexType::Color);
            Add_Textture_For_Mesh_Object(amphora, "./normal_maps/5.jpg", TexType::Normal);
        }
