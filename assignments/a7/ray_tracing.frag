...
void initScene() 
{
    camera = Camera(vec3(0, 15, 50), vec3(5, 0, 0), vec3(0, 3, -3), vec3(-2.5, -1, -1));

    // Floor Material 
    materials[0].ka = vec3(0.1);
    materials[0].kd = vec3(0.48, .23, .12);
    materials[0].ks = vec3(0.6);
    materials[0].shininess = 80.0;
    materials[0].kr = 0.5 * materials[0].ks;

    //// Sphere 0 Material
    materials[1].ka = vec3(0.1);
    materials[1].kd = vec3(0.91);
    ...
    materials[2].kd = vec3(0.9, 0.32, 0.2);
    ...
    materials[3].kd = vec3(0.89, 0.26, 0.7);
    materials[3].ks = vec3(0.8);
    materials[3].shininess = 150;
    materials[3].kr = 0.5 * materials[3].ks;

    lights[0] = Light(vec3(-3, 5, 0.5), 
                            /*Ia*/ vec3(0.1, 0.1, 0.1), 
                            /*Id*/ vec3(1.0, 1.0, 1.0), 
                            /*Is*/ vec3(0.85, 0.85, 0.7));
    lights[1] = Light(vec3(0, 4, 1), 
                            /*Ia*/ vec3(0.1, 0.1, 0.1), 
                            /*Id*/ vec3(0.9, 0.9, 0.9), 
                            /*Is*/ vec3(0.5, 0.5, 0.3));
    planes[0] = Plane(vec3(0, 1, 0), vec3(0, 0, 0), 0);

    spheres[0] = Sphere(vec3(0, 1, -1.5), 0.6, 1);
    spheres[1] = Sphere(vec3(1.1, 0.8, -0.9), 0.4, 2);
    spheres[2] = Sphere(vec3(-1.2, 0.6, -0.8), 0.5, 3);
}
...
vec3 shadingPhong(Light light, int matId, vec3 e, vec3 p, vec3 s, vec3 n) 
{
    ...
    vec3 Id = light.Id;
    vec3 lambertian = sampleDiffuse(matId, p) * Id * max(0, dot(l, n));
...
bool isShadowed(Light light, Hit h) 
{
    ...
    Ray shadow_ray = Ray(intersect + dir * Epsilon, dir);

    float t = findHit(shadow_ray).t;
