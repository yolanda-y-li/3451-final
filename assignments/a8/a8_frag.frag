...
vec3 renderStars(vec2 fragPos)
{
    ...
    for(float i = 0.; i < NUM_STAR; i++){ // for each star
        vec2 pos = hash2d(i) * 2. - 1.; // generate a position for the star
        float brightness = .0004;

        /* your implementation starts */
        brightness *= 0.5 + 0.5 * (sin((hash1d(i) + 5.) * t + hash1d(i)) + 1.); // twinkling
        vec3 color = vec3(1.);
...
vec2 moveParticle(vec2 initPos, vec2 initVel, float t)
{
    ...
    currentPos = initPos +  initVel * t - 0.5 * Gravity * vec2(0.0, t * t);
...
vec3 simSingleFirework(vec2 fragPos, vec2 launchPos, vec2 launchVel, float t, vec3 color)
{
    ...
    if(t < emitTime){ // before emission, show single particle
        ...
        vec2 emitPos = moveParticle(launchPos, launchVel, emitTime); // boss particle position

        for(float i = 0.; i < NUM_EMISSION; i++){ // for each emission particle
            vec2 emitVel = hash2d_polar(i) * .7; // random direction with max magnitude 0.7

            /* your implementation starts */
            float brightness = 0.0004 * (0.5 * cos(1 - pow(2.5, (hash1d(i) + 3.) * emitT + hash1d(i))) + 1.0) * pow(0.6, emitT);
...
vec3 simHeartFirework(vec2 fragPos, vec2 launchPos, vec2 launchVel, float t, vec3 color, float size)
{
    vec3 fragColor = vec3(0.0);
    float emitTime = 1.5;
    ...
    if(t < emitTime){ // before emission, show single particle
        ...
        for(float i = 0.; i < NUM_EMISSION; i++){ // for each emission particle
            float num = 2 * 3.1415 * i / NUM_EMISSION;
            float x = sqrt(2) * pow(sin(num), 3);
            vec2 emitVel =  (size * 0.2 + 0.2) * vec2(x, y);
...
vec3 renderFireworks(vec2 fragPos)
{
    ...
    for(float i = 0.; i < NUM_FIREWORKS; i++){
        ...
        if (int(i) % 4 == 0) {
            float size = hash1d(i);
            fragColor += simHeartFirework(fragPos, launchPos, launchVel, t, color, size);
         } else {
